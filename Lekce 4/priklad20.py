...
Moduly v Pythonu se často snaží zpříjemnit život programátorům. Například je občas otrava vymýšlet jména nebo adresy,
když chceme vyzkoušet, jestli náš program funguje. Jindy třeba potřebujeme nějaká data anonymizovat, tj. odebrat z nich
citlivé osobní údaje jako jména, adresy atd. Pro tento účel existuje modul Faker, která nám umí vygenerovat jména, adresy
 a řadu dalších dat, které můžeme využít při testování našich programů.
...
Objekt generator_falesnych_dat má naprogramované funkce jako name() pro vygenerování náhodného jména, address pro vygenerování
náhodné adresy atd.
...
Tam najdeš informaci, že "faker.Faker also supports multiple locales".
Podívej se, jak je v příkladu vytvořen objekt Faker (je použit seznam).
